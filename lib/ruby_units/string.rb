...
#       Time.local(*ParseDate.parsedate(self))
      Time.local(Date._parse(self, false).values_at(:year, :mon, :mday, :hour, :min, :sec))
    end
  end
...
#       r=DateTime.civil(*ParseDate.parsedate(self)[0..5].compact)
      r=DateTime.civil(Date._parse(self, false).values_at(:year, :mon, :mday, :hour, :min, :sec)[0..5].compact)
    end
...
#       r = Date.civil(*ParseDate.parsedate(self)[0..5].compact)
      r = Date.civil(Date._parse(self, false).values_at(:year, :mon, :mday, :hour, :min, :sec)[0..5].compact)
    end
